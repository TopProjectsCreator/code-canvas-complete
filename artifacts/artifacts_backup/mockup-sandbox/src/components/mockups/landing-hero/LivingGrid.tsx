...
export function LivingGrid() {
  ...
  const [users, setUsers] = useState(1847);
  const [canvases, setCanvases] = useState(892);
  const [hovered, setHovered] = useState<string | null>(null);

  useEffect(() => {
    const userIv = setInterval(() => setUsers((v) => v + (Math.random() > 0.5 ? 1 : -1)), 2200);
    const canvasIv = setInterval(() => setCanvases((v) => v + (Math.random() > 0.5 ? 1 : 0)), 3100);
    return () => {

...
export default function GlobalWhiteboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // stray arrow that shows up above the top card, so it is removed.
      const cardRects = new Map<string, Rect>();
      for (const el of rawElements as any[]) {
        if (el?.isDeleted) continue;
        if (el?.customData?.kind === 'thread-card' || (el?.type === 'rectangle' && !el?.customData)) {
          cardRects.set(el.id, { x: el.x || 0, y: el.y || 0, w: el.width || 0, h: el.height || 0 });
        }
      }
      const pointsIntoCard = (el: any): boolean => {
        if (el?.type !== 'arrow' || el.isDeleted) return false;
        const boundId = el.endBinding?.elementId as string | undefined;
        const pts = Array.isArray(el.points) && el.points.length ? el.points[el.points.length - 1] : [0, 0];
        const endX = (el.x || 0) + (pts[0] || 0);
        const endY = (el.y || 0) + (pts[1] || 0);
        for (const [id, r] of cardRects) {
          if (boundId && boundId !== id) continue;
          const hit =
            endX >= r.x - 16 && endX <= r.x + r.w + 16 && endY >= r.y - 16 && endY <= r.y + r.h + 16;
          if (hit && (el.y || 0) <= r.y + 4) return true;
        }
        return false;
      };
      const strayArrows = (rawElements as any[]).filter(pointsIntoCard);
      const strayIds = new Set(strayArrows.map((el) => el.id));
      const elements = strayIds.size
        ? rawElements.filter((el: any) => !strayIds.has(el.id))
        : rawElements;
      ...
      // Repair stale cards. A card is generated by whichever client happens to be
      // on the board, so a thread posted while nobody is here is first drawn on a
      // later load — and if its content (an uploaded image, an edit) changed after
      // the card was drawn, nothing ever refreshed it. Cards therefore carry a
      // fingerprint of the content they were built from; a mismatch, or an image
      // whose binary never made it into the saved files map, forces a rebuild.
      const savedFileIds = new Set(Object.keys(scene.files || {}));
      const threadCards = new Map<string, any>();
      for (const el of elements as any[]) {
        if (el?.customData?.kind === 'thread-card' && !el.isDeleted) {
          threadCards.set(el.customData.threadId as string, el);
        }
      }
      const staleThreads = new Map<string, { x: number; y: number }>();
      for (const t of threads) {
        const card = threadCards.get(t.id);
        if (!card) continue;
        const owned = (elements as any[]).filter(
          (el) => el?.customData?.threadId === t.id && !el.isDeleted
        );
        const orphanImage = owned.some(
          (el) => el.type === 'image' && el.fileId && !savedFileIds.has(el.fileId)
        );
        if (card.customData?.fp !== threadFingerprint(t) || orphanImage) {
          staleThreads.set(t.id, { x: card.x || 0, y: card.y || 0 });
        }
      }

      let workingElements = elements;
      if (staleThreads.size) {
        const dropIds = new Set<string>();
        const dropGroups = new Set<string>();
        for (const el of elements as any[]) {
          if (!el?.customData?.threadId || !staleThreads.has(el.customData.threadId)) continue;
          dropIds.add(el.id);
      ...
      if (additions.length || staleThreads.size) {

...
export const CodeEditor = ({ file, currentFilePath, onContentChange, collab }: CodeEditorProps) => {
  ...
  const syncPresenceFromSelection = useCallback(() => {
    ...
    if (lineAttr) {
      const nextLine = Number(lineAttr);
      setSelectedLine((prev) => {
        if (prev !== nextLine) setIsCommentPanelOpen(false);
        return nextLine;
      });
    }
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden bg-editor">
      ...
      <div className="flex min-h-0 flex-1">
        <div className="flex min-w-0 flex-1 overflow-auto ide-scrollbar">
          <div className="flex min-h-full min-w-full">
            ...
            <div className="relative min-w-0 flex-1">
              ...
              {selectedLine !== null && (
                <div className="absolute right-4 z-20" style={{ top: `${Math.max((selectedLine - 1) * 24 - 8, 8)}px` }}>
                  <Button
                    type="button"
                    size="sm"
                    variant={selectedLineThreads.length > 0 ? 'secondary' : 'default'}
                    className="h-8 gap-1 rounded-full px-3 text-xs shadow-lg"
                    onClick={() => setIsCommentPanelOpen(true)}
                  >
                    {selectedLineThreads.length > 0 ? <MessageSquare className="h-3.5 w-3.5" /> : <MessageSquarePlus className="h-3.5 w-3.5" />}
                    {selectedLineThreads.length > 0 ? `Open thread (${selectedLineThreads.length})` : 'Comment'}
                  </Button>
                </div>
              )}
              <div
        ...
        {collab && currentFilePath && selectedLine !== null && isCommentPanelOpen && (
            ...
            <div className="border-b border-border px-4 py-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button type="button" variant="ghost" size="icon" className="h-7 w-7" onClick={() => setIsCommentPanelOpen(false)}>
                    <X className="h-4 w-4" />
                  </Button>
                </div>
              </div>

        ...
        {messages.map((message) => (
          ...
          >
            ...
            >
              {message.role === 'assistant' ? (
                <div className="space-y-2">
                  {/* Route label: proves whether this reply came from the local model or the cloud */}
                  {message.modelSource && (
                    <div className="flex items-center gap-1 text-[10px] text-muted-foreground" title={message.modelSource === 'local' ? 'Generated on-device by your selected offline model' : 'Generated by the cloud model'}>
                      {message.modelSource === 'local'
                        ? <><WifiOff className="w-3 h-3 text-emerald-400" /><span>Local model</span></>
                        : <><Sparkles className="w-3 h-3 text-primary" /><span>Cloud</span></>}
                    </div>
                  )}
